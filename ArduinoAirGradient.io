...
String APIROOT = "http://192.168.5.2:8080";
...
void loop() {
  ...
  String payloadPM2;
  String payloadCO2;
  String payloadTEMPHUM;

  if (hasPM) {
    int PM2 = ag.getPM2_Raw();
    payload = payload + "\"pm02\":" + String(PM2);
    payloadPM2 = String(PM2);
    //Serial.println(payloadPM2);
  ...
  if (hasCO2) {
    ...
    payloadCO2 = String(CO2);
    //Serial.println(payloadCO2);
  ...
  if (hasSHT) {
    ...
    payloadTEMPHUM = String(result.t) + ";" + String(result.rh) + ";0";
    //Serial.println(payloadTEMPHUM);
  ...
  // send payload "AirGradient PM (custom) IDX=7739
  if (connectWIFI) {
    Serial.println(payloadPM2);
    //String POSTURL = APIROOT + "sensors/airgradient:" + String(ESP.getChipId(), HEX) + "/measures";
    String POSTURL = APIROOT + "/json.htm?type=command&param=udevice&idx=7739&nvalue=0&svalue=" + payloadPM2;
...
//// send payload AirGradient CO2 IDX=7741
//if (connectWIFI) {
//    Serial.println(payloadCO2);
//    //String POSTURL = APIROOT + "sensors/airgradient:" + String(ESP.getChipId(), HEX) + "/measures";
//    String POSTURL = APIROOT + "/json.htm?type=command&param=udevice&idx=7741&nvalue=" + payloadCO2;
//    Serial.println(POSTURL);
//    WiFiClient client;
//    HTTPClient http;
//    http.begin(client, POSTURL);
...
// send payload AirGradient Temp Hum IDX=7740
if (connectWIFI) {
    Serial.println(payloadTEMPHUM);
    //String POSTURL = APIROOT + "sensors/airgradient:" + String(ESP.getChipId(), HEX) + "/measures";
    String POSTURL = APIROOT + "/json.htm?type=command&param=udevice&idx=7740&nvalue=0&svalue=" + payloadTEMPHUM;
    Serial.println(POSTURL);
    WiFiClient client;
    HTTPClient http;
    http.begin(client, POSTURL);
